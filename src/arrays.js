...
start- начальный индекс позиции источника, откуда начинать копировать элементы.
  ...
  */
  ...
  const ass = [7, 8, 3, 4, 5, 6];
  console.log(typeof ass); // object
  console.log(Array.isArray(ass)); // true
  console.log(Array.of(1, 2, 3));//[1,2,3] массив из
  const [y, x] = ass;
  console.log('деструкторизация в массив - ', y, x)
  ...
  let fruits = [];
  let arrToObj = ["value1", "value2", "value3"];
  let arrObject = { ...arrToObj };
  console.log(arrObject);//{ '0': 'value1', '1': 'value2', '2': 'value3' }
  console.log(Object.values(arrObject));//[ 'value1', 'value2', 'value3' ]
  console.group("********* стэк");
  ...
  console.log('----сортирует без учета большие маленькие - ', animals); // 'АИСТ', 'ёж', 'енот', 'ехидна', 'тигр', 'ЯК'
  console.log('----сортирует сначала большие потом маленькие - ', animals.sort()); //'АИСТ', 'ЯК', 'енот', 'ехидна', 'тигр', 'ёж'
  //---------
  const arrNum = [4, 6, 19, 34, 32];
  console.log('----сортирует как строки - ', arrNum.sort()); //[ 19, 32, 34, 4, 6 ] // ! сортирует как строки
